...
public class addsp extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet addsp</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet addsp at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            nhanvatDAO.addCharacter(name,title);

            // Gửi thông báo thành công và chuyển hướng về trang list
            HttpSession session = request.getSession();
            session.setAttribute("successMsg", "Đã thêm nhân vật " + name + " vào cơ sở dữ liệu.");
